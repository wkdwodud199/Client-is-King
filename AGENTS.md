...
## 게임 task 설계 기준 (task-101+)

게임 task(`task-101` 이후)를 설계할 때는 다음을 전제로 범위를 판단한다:

- 컨셉·로드맵·아키텍처 규약 SSOT: `kb/concepts/project-brief.md` — 브리프를 재정의하는 설계를 만들지 않는다.
- 데모 스코프 가드: `kb/concepts/demo-scope.md` — 주차장 항목(Steam 연동, 직원 고용, 인테리어, 다점포,
  미슐랭 트랙, 조리 미니게임)은 설계에 포함하지 않고, 하드캡을 넘는 범위를 요구하지 않는다.
- Unity 구현 대상 경로는 `game/` 하위로 지정한다 (task 기록은 `kb/` — 이 경계는 설계에도 그대로 적용된다).
