...
## v2 재분류 (2026-07-12 — task-116)

오너가 2026-07-12 Codex Visual North Star(현대 뉴욕 코리아타운 푸드트럭) 콘셉트를 **최종 아트 방향**으로
승인했다. 이 문서가 기록한 CC0 combo 1(Ninja Adventure + karsiori + Henry + Kenney) 디렉션은 이제
**프로토타입(데모 출시본)** 으로 재분류한다 — task-114/115가 이 디렉션으로 완료·커밋된 데모 마감본이며,
`Placeholders/**`·`OpenSource/**`는 무수정 보존된다(task-116 설계 G절).

- **프로토타입 디렉션 (이 문서 본문, 아래 원본 유지)**: CC0/OFL 에셋 기반, 데모 출시본의 비주얼. 계속 유효 —
  대체가 아니라 병행 보존.
- **최종 디렉션 (신규)**: NYC 코리아타운 아트 오버홀 — 오너 승인 콘셉트 원본은 `kb/concepts/art-originals/`
  (6종, PROVENANCE.md md5 핀 보존)에 아카이브되어 있고, 런타임 적용 계약은
  [task-116 design.md](../tasks/task-116/design.md)가 정한다(경로·규격·정책·테스트 기준 — 콘셉트 자체의
  재디자인 없음).
- **전환 방식**: `SceneBuilder`의 스프라이트 소스 경로 전환(단일 커밋, task-116 U3)으로 이행하며, 반려·회귀
  시 그 커밋 하나로 프로토타입 디렉션(이 문서 본문)으로 즉시 복귀 가능하다.

이하 본문은 v1(프로토타입 디렉션) 원본을 그대로 보존한다.
