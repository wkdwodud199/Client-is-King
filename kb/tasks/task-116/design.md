...
- **신규 표현 슬롯 (배치 2 — 별도 결정)**: 주인공 스프라이트(현 런타임에 슬롯 없음), 시스템 아이콘 8종 UI 배선(현 UI는 텍스트 기반), 손님 표정 프레임, 증기 이펙트, 트럭 독립 소품화. 콘셉트에는 존재하지만 런타임 슬롯 신설 = 표현 시스템 확장이므로 오픈 이슈 4로 라우팅(승인 시 task-118+ — task-117은 인게임 크레딧 UI, 2026-07-13 정정).
...
4. **[비차단] 배치 2 범위 (신규 표현 슬롯)**: 주인공 스프라이트·시스템 아이콘 8종 UI 배선·손님 표정·증기 이펙트·트럭 독립 소품. 콘셉트에는 존재하나 런타임 슬롯 신설이 필요해 배치 1에서 제외했다. 승격 시 별도 task-118+ 설계(오너 결정 — 이 표에 기록만; task-117은 인게임 크레딧 UI가 사용, 2026-07-13 정정). 배치 1 진행을 막지 않는다.
