...
> Next step: Claude가 U0(SSOT 개정 커밋)→U1(계약 코드) 순으로 구현에 착수한다. 입력 패키지(배치 1) 전달 게이트 + 공개 표기 문구 확정(오픈 이슈 5)을 거쳐 U2~U4를 진행하고, 시각 승인 게이트(H절) 통과 후 Codex 코드리뷰로 마감한다.
...
### C. AI 아트 사용 정책 + provenance (계약 — 공개 표기 문구만 오너 확정 대기: 오픈 이슈 5, U2 게이트)
...
- **공개 표기**: 리포가 public이므로 커밋 즉시 게시다. Codex(gpt-image, OpenAI) 생성물임을 리포와 게임 크레딧에 표기하는 방향을 제안하되, **문구·위치·라이선스 선언(저작권 지위 불확실성 고지 포함)은 오너가 확정한다**(오픈 이슈 5). 이 확정은 **U2(입력 패키지 커밋)의 게이트**다 — 확정 전에는 입력 패키지를 커밋하지 않으며, U0/U1 진행은 차단하지 않는다.
- **CC0 자산과의 구획**: 기존 `PLACEHOLDER-PROVENANCE.md`(CC0 계보)와 NYC provenance(AI 계보)는 파일·디렉터리 수준에서 분리한다 — 라이선스 체계가 다른 자산을 한 문서에 섞지 않는다.

### D. 최종 파일 경로 + Asset Map (확정 — Codex 교차검토 승인. 잔여: 장르 라벨 의미 검증 = 오픈 이슈 9, 생산 측 비차단)
...
| Customers/family_parent* | customers.png 3열 | 그린 패딩 부모 + 옐로 패딩 아이 페어 | 캔버스 32×32 확정 — 페어/부모 단독 구도는 캔버스 내 생산자(오너/Codex) 아트 콘텐츠 결정 (이슈 2 해소) |
...
- 기각된 대안(결정 기록): 손님 16×16 유지(×4 표시 — 디테일 손실)와 family_parent 페어의 48×32 캔버스(rect 조정 = 유일한 씬 변경 유발 지점)는 기각. **family_parent는 캔버스 32×32로 락**(rect·씬 무변경) — 페어(부모+아이) vs 부모 단독은 32×32 캔버스 안에서 스프라이트 생산자(오너/Codex)가 정하는 **아트 콘텐츠 결정**이지 씬/규격 결정이 아니다(Claude는 콘텐츠를 결정하지 않는다). 이로써 오픈 이슈 2 해소.
...
3. **RGB 납품 시 폴백(키잉 — 허용 확정)**: 생산 도구가 알파를 출력하지 못하면, **납품자가 지정한 단일 배경색의 exact-match 키잉**(해당 RGB만 alpha 0, 그 외 무변경)만 결정론 후처리로 허용한다. 배경색·적용 파일을 provenance에 기록하고, 경계 헤일로가 남으면 기계 검증이 아닌 시각 게이트에서 반려한다. (오픈 이슈 3 해소)
...
- **신규 `NycArtTests.cs` (EditMode — U2에서 자산과 함께 커밋, 커밋 상태에서 항상 green)**:
  ...
  7. 원본 보존: `kb/concepts/art-originals/` 6종 md5가 PROVENANCE.md의 md5 핀(2026-07-12 선행 추가됨)과 일치(콘셉트 불변 증거).
...
4. U2 — 공개 표기·라이선스 문구의 오너 확정(오픈 이슈 5 — U2 커밋 게이트)을 확인한 뒤, 합격 패키지를 `game/Assets/Art/NYC/`에 수입(+`.meta`), provenance 작성, `NycArtTests` 추가, EditMode green 확인.
...
| U2-intake-validate | `game/Assets/Art/NYC/**` 32파일+`.meta`+`NYC-ART-PROVENANCE.md`, `game/Assets/Tests/EditMode/NycArtTests.cs` 신설 | U1-contract + 입력 패키지 전달 게이트(오너/Codex) + 공개 표기 문구 확정(오픈 이슈 5) | G2 |
...
| `kb/concepts/art-originals/*.png` (6종) | none | PNG 바이트 불변 — PROVENANCE.md의 md5 핀으로 검증(H7; md5 열은 2026-07-12 본 설계 교차검토에서 선행 추가됨) |
...
번호는 본문 상호참조 보존을 위해 초안 그대로 유지한다. 1·2·3·7은 **확정으로 닫힘**(결정 기록), 5는 **U2 커밋 게이트**, 나머지는 **배치 1 구현 비차단**이다.
...
2. **[확정 — 닫힘] 픽셀 규격**: E절 표 확정(손님 32×32 · 음식 32×32 · 장르 아이콘 32×32 · backdrop 640×160 · counter 320×32 — 전부 rect 정수 약수, 씬 rect 무변경). **family_parent 캔버스 32×32 락** — 페어/부모 단독 구도는 캔버스 내 생산자(오너/Codex)의 아트 콘텐츠 결정이지 씬/규격 결정이 아니다(Claude 미결정).
3. **[확정 — 닫힘] 알파 정책**: RGBA 실투명 배경 + 이진 알파(a∈{0,255}, backdrop 불투명 예외) + RGB 납품 시 exact-match 키잉 폴백 허용 (F1/F3 확정 — Codex 적합 판정).
...
5. **[U2 커밋 게이트] AI 아트 공개 표기·라이선스 선언 문구**: public 리포 게시·게임 크레딧·Steam 정책 대응 문구는 오너 확정 사안. **입력 패키지 커밋(U2)의 게이트**이며 U0/U1 진행은 차단하지 않는다(C절).
6. **[비차단] 무대 배경 방식**: 640×160 단판 합성(권고 유지 — 콘셉트 장면 재현) vs NYC 타일셋 반복(트럭 별도 소품화 필요, 배치 2 연동). 단판 채택 시 Image type Tiled→Simple 전환이 SceneBuilder에 발생(U3 포함). 늦어도 입력 패키지 생산 착수 전 Codex 확정 — U0/U1은 막지 않는다.
...
9. **[비차단] 장르 아이콘 의미 매핑 검증**: D절의 "꼬치=분식·도시락=제네럴리스트" 등은 이 설계의 추정 라벨이다 — 입력 패키지 생산 전 Codex가 콘셉트 의도 기준으로 확정한다(생산자 측 확인 사항 — Claude 구현은 id 기준이라 영향 없음).
10. **[비차단] 좌향 플립의 비대칭 디테일**: 백팩·토트백이 좌향 이동 시 반전되어 보이는 현행 규약(localScale 플립)의 허용 여부 — Codex 확인, 불허 시 좌향 프레임 별도 생산(입력 패키지 확장 — 배치 1 스펙은 현행 플립 유지).

---

**결정 확정 기록 (2026-07-12)**: 오너가 B절 A안(타깃 해상도 재생산)을 채택하고, Codex 설계 교차검토가 픽셀 규격(E절)·알파/프레임 스펙(F절)을 적합 판정했으며, 콘셉트 원본 md5 핀이 `PROVENANCE.md`에 추가되어 H7 검증이 구현 가능해졌다. 이로써 초안의 ready 승격 전제(오픈 이슈 1~3·7)가 전부 해소되어 Status를 ready로 승격한다. 잔여 오픈 이슈는 배치 1 구현을 차단하지 않으며(5번만 U2 커밋 게이트), 구현 중 디자인 판단이 필요하면 추측하지 않고 Codex 검수로 돌린다.
